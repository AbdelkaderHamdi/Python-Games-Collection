...
def get_number_of_lines():
    while True:
        lines= input(f"Enter the number of linesto bet on (1-{MAX_LINES})? ")
        if lines.isdigit():
            lines= int(lines)
...
def get_bet():
    while True:
        bet= input("what would you like to deposit on each line? $")
        if bet.isdigit():
            bet= int(bet)
...
def spin(balance):
    ...
    while True: 
        ...
        if total_bet >balance: 
            print(f"You don't have enough to bet that amount , your balance: {balance}. ")
        else: 
            break
